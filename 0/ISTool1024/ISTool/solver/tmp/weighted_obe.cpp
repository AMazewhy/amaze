...
namespace {
    ...
    FunctionContext _weightedEnumerate(const std::vector<PSynthInfo> &info_list, const EnumConfig &c) {
        ...
        for (int size = 1;; ++size) {
                ...
                for (auto* symbol: direct_order_list[pos]) {
                    ...
                    if (symbol->name != start->name){
                        ...
                        for (auto* rule: start->rule_list) {
                            if (_isDirectRule(rule) && rule->param_list[0]->name == symbol->name){
                                gap = 0;
                                break;
                            }
                            bool have_aim = false;
                            for (auto* nt:rule->param_list){
                                if (nt->name == symbol->name){
                                    have_aim = true;
                                    break;
                                }
                            }
                            if (have_aim){
                                int newgap = _getWeight(rule);
                                if (newgap<gap){
                                    gap = newgap;
                                }
                            }
                        }
                        if (gap == 100){//ite B S S 被提取为N6后 B找不到到S的距离
                            //LOG(INFO) << "symbol name " << symbol->name;
                            gap = 0;
                        }
                        //assert(gap!=100);
                        MaxSize = MaxSize - gap;
                    ...
                    for (auto* rule: symbol->rule_list) {
                        if (_isDirectRule(rule)) {
                            ...
                            ProgramStorage tmp = merge(sub_id_list, MaxSize - _getWeight(rule), storage_list);
                            for (const auto &sub_list: tmp) {
                                ...
                                if (o->isDuplicated(info->name, symbol, p))
                                    continue;
                                storage_list[id][MaxSize].push_back(p);
                            }
                        }
                    }
                }
            ...
            std::vector<int> sub_id_list;
            for (const auto& info: info_list) {
                auto* start = info->grammar->start;//start symbol
            ...
            for (const auto& sub_list: tmp) {
                ...
                if (v->verify(info, nullptr)) {//计数k次 先检验再去重
